...
const Ghosts = React.createClass({
    ...
    createGhost(){
        axios.post('http://localhost:8080/pa165/ghosts', {
            ...
            hauntedHouse : this.state.defaultHouse,
    ...
    createHaunting(ghost){
        axios.post('http://localhost:8080/pa165/hauntings', {
            ...
            .then(res => {
                console.log("Haunting " + res.data + " was created");
                const object = this.state.ghosts;
                object.push(res.data);
                this.setState({ghosts: object});
            })
...
*/
  componentDidMount: function() {
    ...
    $.ajax({
      url: 'http://localhost:8080/pa165/houses/5',
      dataType: 'json',
      cache: false,
      success: function(data) {
        this.setState({defaultHouse: data});
      }.bind(this),
      error: function(xhr, status, err) {
        console.error(this.props.url, status, err.toString());
      }.bind(this)
