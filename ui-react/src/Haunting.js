...
const Hauntings = React.createClass({
  ...
  componentDidMount() {
    axios.get(`http://localhost:8080/pa165/hauntings`)
      .then(res => {
        ...
        this.setState({hauntings: houses});
        console.log(this.state.hauntings);
      });
  ...
  render() {
    ...
    this.state.hauntings.forEach(function (haunting, i) {
      ...
      let names = '';
      haunting.ghosts.forEach(function (ghost, i) {
        if (i == 0) {
          names = ghost.name
        }
        names = names.concat(', '.concat(ghost.name));
      });
      tableLines.push(
        <tr key={i}>
          <th>{ haunting.id }</th>
          ...
          <Modal.Body>
            <Form horizontal>
              ...
              <FormGroup controlId="hauntingNumberOfPeoplePresent">
                ...
                <Col sm={7}>
                  <FormControl type="text" placeholder="Haunting number of people present"
                               value={this.state.numberOfPeoplePresent}
